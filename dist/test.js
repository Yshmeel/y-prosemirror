(function () {
  ...
  */
   ...
   * @template T
   */
  const oneOf$1 = (gen, array) => array[int31(gen, 0, array.length - 1)];
  ...
  class TestConnector {
    ...
    flushRandomMessage () {
      ...
      if (conns.length > 0) {
        const receiver = oneOf$1(gen, conns);
        const [sender, messages] = oneOf$1(gen, Array.from(receiver.receiving));
    ...
    disconnectRandom () {
      ...
      oneOf$1(this.prng, Array.from(this.onlineConns)).disconnect();
      return true
    }
    ...
    reconnectRandom () {
      ...
      oneOf$1(this.prng, reconnectable).connect();
      return true
    }
  }
        ...
        return {
          update: () => {
            ...
            if (pluginState.snapshot == null && pluginState.prevSnapshot == null) {
              if (changedInitialContent || !equalityDeep(view.state.doc.type.createAndFill().content.toJSON(), view.state.doc.content.toJSON())) {
  ...
  const equalYTextPText = (ytext, ptexts) => {
    ...
    return delta.length === ptexts.length && delta.every((d, i) => d.insert === /** @type {any} */ (ptexts[i]).text && keys(d.attributes || {}).length === ptexts[i].marks.length && ptexts[i].marks.every(mark => equalAttrs$1(d.attributes[mark.type.name] || {}, mark.attrs)))
  };
   ...
   * @return {Y.Doc}
   */
  function prosemirrorToYDoc (doc, xmlFragment = 'prosemirror', ydoc) {
   ...
   * @return {Y.Doc}
   */
  function prosemirrorJSONToYDoc (schema, state, xmlFragment = 'prosemirror', ydoc) {
  ...
  const emDOM$1 = ['em', 0];
  const strongDOM$1 = ['strong', 0];
  const codeDOM$1 = ['code', 0];
  ...
  // To reuse elements from this schema, extend or read from its
  // `spec.nodes` and `spec.marks` [properties](#model.Schema.spec).
  const schema$1 = new Schema({ nodes: nodes$1, marks: marks$1 });

  const schema$2 = /** @type {any} */ (schema);
  ...
  const testDocTransformation = tc => {
    ...
    view.dispatch(view.state.tr.insert(0, /** @type {any} */ (schema$2.node('paragraph', undefined, schema$2.text('hello world')))));
    const stateJSON = view.state.doc.toJSON();
    // test if transforming back and forth from Yjs doc works
    const backandforth = yDocToProsemirrorJSON(prosemirrorJSONToYDoc(/** @type {any} */ (schema$2), stateJSON));
    compare(stateJSON, backandforth);
  };
  ...
  const marksChoices = [
    [schema$2.mark('strong')],
    [schema$2.mark('em')],
    [schema$2.mark('em'), schema$2.mark('strong')],
    [],
    []
  ];
  ...
  const pmChanges = [
    ...
    (y, gen, p) => { // insert text
      ...
      const marks = oneOf$1(gen, marksChoices);
      const tr = p.state.tr;
      const text = charCounter++ + word(gen);
      p.dispatch(tr.insert(insertPos, schema$2.text(text, marks)));
    },
    /**
     * @param {Y.Doc} y
     * @param {prng.PRNG} gen
    ...
    (y, gen, p) => { // insert paragraph
      ...
      const tr = p.state.tr;
      const text = charCounter++ + word(gen);
      p.dispatch(tr.insert(insertPos, schema$2.node('paragraph', undefined, schema$2.text(text, marks))));
    },
    /**
     * @param {Y.Doc} y
     * @param {prng.PRNG} gen
    ...
    (y, gen, p) => { // insert codeblock
      ...
      const tr = p.state.tr;
      const text = charCounter++ + word(gen);
      p.dispatch(tr.insert(insertPos, schema$2.node('code_block', undefined, schema$2.text(text))));
    },
    /**
     * @param {Y.Doc} y
     * @param {prng.PRNG} gen
    ...
    (y, gen, p) => { // wrap in blockquote
      ...
      const wrapping = range && findWrapping(range, schema$2.nodes.blockquote);
      if (wrapping) {
